...
@Validated
@Tag(name = "studentResult", description = "the studentResult API")
...
public interface StudentResultApi {
    ...
    @Operation(
        ...
        tags = { "student-result-controller" },
        responses = {
            @ApiResponse(responseCode = "200", description = "OK", content = {
                @Content(mediaType = "*/*", schema = @Schema(implementation = StudentResult.class))
            })
        }
    )
    @RequestMapping(
    ...
    default ResponseEntity<List<StudentResult>> getAllStudentResult(
        
    ) {
        getRequest().ifPresent(request -> {
            for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf("*/*"))) {
                    String exampleString = "{ \"exam\" : \"exam\", \"rollno\" : 6, \"marks\" : 0 }";
